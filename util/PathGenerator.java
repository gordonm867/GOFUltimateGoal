...
public class PathGenerator implements Callable<ArrayList<Point[]>> {
    ...
    public ArrayList<Point[]> getPath(int rings) {
        ...
        if(rings == 0) {
            ...
            path.add(new Line(new Point(-3, -0.2), new Point(-3.45, 0.6, 180))); // Power shots-wobble drop off
            ...
            path.add(new Line(new Point(blue ? Globals.START_X : -Globals.START_X, Globals.START_Y), new Point(-3, -0.2, 104.6875))); // Start-power shots
            path.add(new Line(new Point(-3, -0.2), new Point(-4, 3.4, -135), obstacles)); // Power shots-wobble drop off
            path.add(new Line(new Point(-4.6, 3.4), new Point(-2.3, -2.2, 68), fullobstacles)); // Wobble drop off-wobble pickup
            path.add(new Line(new Point(-2.16, -2.58), new Point(-3.12 - (-0.5/12), -3.15, 90), obstacles));
