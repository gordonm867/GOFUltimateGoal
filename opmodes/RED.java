...
public class RED extends MyOpMode {
    ...
    public void loopOp() {
        ...
        if(index == 1) {
            ...
            if(System.currentTimeMillis() - i1timer >= 1000 && odometry.getPoint().distance(path.get(index)[path.get(index).length - 1], Unit.FEET) < 0.3) {
            ...
            if(displacement > 1.0/20.0 && !(rings == 1 && index == 2 && odometry.getY() < path.get(index)[path.get(index).length - 1].getY())) {
                ...
                while(opModeIsActive() && System.currentTimeMillis() - timer <= 500) {
                    ...
                    if(index == 0 && rings != 4) {
                        ...
                        drive.lasterror = 0;
                        while((displacement > 5.0/96.0 || Math.abs(angle - 90.0 /* angle 1 */) > 1.5)) {
                            ...
                            data = robot.bulkRead();
                            displacement = odometry.getPoint().distance(new Point(0.2, 0), Unit.FEET);
                            drive.update(robot, new Point(0.2, 0), odometry, 90.0 /* angle 1 */, angle, data);
                            if(Math.abs(odometry.getVelocity()) < 0.1 / 1000.0 && Globals.MIN_SPEED < 0.3) {
                                Globals.MIN_SPEED += 0.0075;
                            }
                            else if(displacement < 1.0/48.0 && Math.abs(angle - 90.0 /* angle 1 */) > 0.6 && Globals.MIN_SPEED > 0.15) {
                                Globals.MIN_SPEED -= 0.0075;
                            }
                            wobble.update(robot, state);
                        }
                        ...
                        drive.lasttime = System.currentTimeMillis();
                        drive.lasterror = 0;
                        while((displacement > 1.0/48.0 || Math.abs(angle - 90.2 /* angle 2 */) > 0.6) || omega > 0.15) {
                            if(rings == 1 && !fishy && !fish && robot.ringsensor.getDistance(DistanceUnit.MM) < 25) {
                                fishy = true;
                                timetostopintakingandgotobed = System.currentTimeMillis();
                            }
                            ...
                            else if(displacement < 1.0/48.0 && Math.abs(angle - 90.2 /* angle 2 */) > 0.6 && Globals.MIN_SPEED > 0.15) {
                                Globals.MIN_SPEED -= 0.0075;
                            }
                            wobble.update(robot, state);
                        }
                        ...
                        drive.lasttime = System.currentTimeMillis();
                        drive.lasterror = 0;
                        while((displacement > 1.0/48.0 || Math.abs(angle - 90.0 /* angle 3 */) > 0.6) || omega > 0.15) {
                            if(rings == 1 && !fishy && !fish && robot.ringsensor.getDistance(DistanceUnit.MM) < 25) {
                                fishy = true;
                                timetostopintakingandgotobed = System.currentTimeMillis();
                            }
                            ...
                            else if(displacement < 1.0/48.0 && Math.abs(angle - 90.0 /* angle 3 */) > 0.6 && Globals.MIN_SPEED > 0.15) {
                                Globals.MIN_SPEED -= 0.0075;
                            }
                            wobble.update(robot, state);
                        }
