...
@Autonomous(name="PARK-RED")
public class PARKRED extends MyOpMode {
    ...
    public void loopOp() throws InterruptedException, GOFException, IOException {
        ...
        while(displacement > 0.1 || Math.abs(angle - 103) > 1) {
            displacement = odometry.getPoint().distance(new Point(odometry.getX(), -0.5), Unit.FEET);
            drive.update(robot, new Point(x, -0.5), odometry, 103, angle, robot.bulkRead());
